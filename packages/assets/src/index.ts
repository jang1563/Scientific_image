...
function createDrugDiscoveryFlagshipTemplateNodes(template: WorkflowTemplate, input: {
  ...
  stages.forEach((stage, index) => {
    ...
    nodes.push(figureShape("round-rect", "", createTransform(sx, stageY, stageW, stageH), {
      ...
      strokeWidth: 1.25,
      depth: index === 4 ? theme.floatingDepth : theme.panelDepth
    }));
    ...
    nodes.push(figureShape("round-rect", "", createTransform(sx + stageW - 78, stageY + 12, 64, 20), {
      fill: warningStage ? theme.warningFill : theme.chipFill,
      stroke: warningStage ? theme.warningStroke : theme.chipStroke,
      strokeWidth: 1,
      depth: "surface"
    }));
    nodes.push(figureText(stage.kind, createTransform(sx + stageW - 66, stageY + 16, 44, 12), {
      fontSize: 8.1,
      fontWeight: 850,
      color: warningStage ? theme.warningText : stage.accent
    }));
    ...
    nodes.push(figureText(stage.label, createTransform(sx + 14, stageY + 124, stageW - 28, 18), {
      fontSize: 12.2,
      fontWeight: 900,
      color: warningStage ? theme.warningText : theme.text
    }));
    nodes.push(figureText(stage.note, createTransform(sx + 14, stageY + 144, stageW - 28, 16), {
      fontSize: 9.4,
      fontWeight: 720,
      color: theme.muted
    }));
  ...
  nodes.push(figureText("Decision spine: validate biology -> screen compounds -> rank hits -> optimize leads -> nominate package", createTransform(x + 24, stageY + stageH + 20, width - 48, 16), {
    fontSize: 9.6,
    fontWeight: 820,
    color: theme.muted
  }));
...
function createSyntheticBiologyFlagshipTemplateNodes(template: WorkflowTemplate, input: {
  ...
  const nodes: SceneNode[] = [
    ...
    figureText("agent-ready DBTL workflow", createTransform(x + 244, y + 17, 226, 18), {
      fontSize: 10.5,
      fontWeight: 850,
      color: theme.accent2
    })
  ...
  const stageW = Math.round((width - 64) / 5);
  const stages = [
    { assetId: "genetic-circuit", label: "Design", kind: "circuit", note: "parts + logic", accent: theme.accent },
    { assetId: "dna-assembly", label: "Build", kind: "assembly", note: "DNA fragments", accent: theme.accent },
    { assetId: "chassis-cell", label: "Chassis", kind: "host", note: "engineered cell", accent: theme.accent2 },
    { assetId: "biosensor-circuit", label: "Test", kind: "readout", note: "reporter response", accent: theme.accent2 },
    { assetId: "kill-switch", label: "Contain", kind: "review", note: "safety gate", accent: theme.isLine ? "#111827" : theme.isDark ? "#fdba74" : "#dc2626" }
  ];
  stages.forEach((stage, index) => {
    const sx = x + index * (stageW + 16);
    const warningStage = index === stages.length - 1;
    ...
    nodes.push(figureShape("round-rect", "", createTransform(sx + stageW - 82, stageY + 12, 68, 20), {
      fill: warningStage ? theme.warningFill : theme.chipFill,
      stroke: warningStage ? theme.warningStroke : theme.chipStroke,
      strokeWidth: 1,
      depth: "surface"
    }));
    nodes.push(figureText(stage.kind, createTransform(sx + stageW - 70, stageY + 16, 48, 12), {
      fontSize: 7.8,
      fontWeight: 850,
      color: warningStage ? theme.warningText : stage.accent
    }));
  ...
  nodes.push(figureText("Decision spine: design circuit -> assemble DNA -> engineer chassis -> test biosensor -> containment review", createTransform(x + 24, stageY + stageH + 20, width - 48, 16), {
    fontSize: 9.5,
    fontWeight: 820,
    color: theme.muted
  }));
...
function createMicrobiomeInfectiousDiseaseFlagshipTemplateNodes(template: WorkflowTemplate, input: {
  ...
  const nodes: SceneNode[] = [
    ...
    figureText("agent-ready surveillance workflow", createTransform(x + 312, y + 17, 258, 18), {
      fontSize: 10.5,
      fontWeight: 850,
      color: theme.accent2
    })
  ...
  const stageW = Math.round((width - 64) / 5);
  const stages = [
    { assetId: "microbiome-community", label: "Community", kind: "ecology", note: "baseline taxa", accent: theme.accent },
    { assetId: "mucosal-barrier", label: "Host barrier", kind: "tissue", note: "mucosal context", accent: theme.accent },
    { assetId: "pathogen-host-interaction", label: "Interaction", kind: "infection", note: "host-pathogen", accent: theme.accent2 },
    { assetId: "metagenomic-read", label: "Sequence", kind: "assay", note: "metagenomics", accent: theme.accent2 },
    { assetId: "antimicrobial-resistance", label: "Review", kind: "AMR gate", note: "risk + outbreak", accent: theme.isLine ? "#111827" : theme.isDark ? "#fdba74" : "#dc2626", profile: theme.riskSymbolProfile }
  ];
  stages.forEach((stage, index) => {
    const sx = x + index * (stageW + 16);
    const warningStage = index === stages.length - 1;
  ...
  nodes.push(figureText("Decision spine: community context -> host barrier -> pathogen interaction -> metagenomic evidence -> AMR/outbreak review", createTransform(x + 24, stageY + stageH + 20, width - 48, 16), {
    fontSize: 9.5,
    fontWeight: 820,
    color: theme.muted
  }));
...
function createCellTherapyFlagshipTemplateNodes(template: WorkflowTemplate, input: {
  ...
  const nodes: SceneNode[] = [
    ...
    figureText("agent-ready release + monitoring path", createTransform(x + 248, y + 17, 258, 18), {
      fontSize: 10.5,
      fontWeight: 850,
      color: theme.accent2
    })
  ...
  const stageW = Math.round((width - 64) / 5);
  const stages: Array<{ assetId: string; label: string; kind: string; note: string; accent: string; profile?: AssetStyleProfile }> = [
    { assetId: "leukapheresis", label: "Collect", kind: "patient", note: "apheresis input", accent: theme.accent },
    { assetId: "viral-vector-transduction", label: "Engineer", kind: "vector", note: "gene transfer", accent: theme.accent },
    { assetId: "cell-expansion", label: "Expand", kind: "culture", note: "manufacturing", accent: theme.accent2 },
    { assetId: "release-testing", label: "Release", kind: "QC gate", note: "potency + safety", accent: theme.accent2 },
    { assetId: "patient-infusion", label: "Monitor", kind: "risk", note: "infusion follow-up", accent: theme.isLine ? "#111827" : theme.isDark ? "#fdba74" : "#dc2626", profile: theme.riskSymbolProfile }
  ];
  stages.forEach((stage, index) => {
    const sx = x + index * (stageW + 16);
    const warningStage = index === stages.length - 1;
  ...
  nodes.push(figureText("Decision spine: patient material -> immune engineering -> expansion -> release QC -> infusion and safety monitoring", createTransform(x + 24, stageY + stageH + 20, width - 48, 16), {
    fontSize: 9.5,
    fontWeight: 820,
    color: theme.muted
  }));
