...
export const PREMIUM_WORKFLOW_PACKS: WorkflowPack[] = [
  ...
  },
  {
    ...
    templates: ["spatial-workflow", "spatial-results-panel", "spatial-results-panel-journal", "spatial-realistic-hybrid-panel", "tissue-to-spot-workflow", "segmentation-neighborhood-figure", "spatial-expression-summary"],
  ...
  },
  {
    ...
    templates: ["ai-biosecurity-pipeline", "ai-biosecurity-pipeline-journal", "permissioning-ladder", "benchmark-dashboard", "review-audit-flow"],
...
export const PREMIUM_WORKFLOW_TEMPLATES: WorkflowTemplate[] = [
  ...
  },
  {
    ...
    description: "Manuscript-safe spatial transcriptomics figure with line-art tissue panels, segmentation/neighborhood evidence, heatmap metadata, and source/provenance checklist.",
    layout: "multi-panel",
    recommendedStyleProfile: "publication-line",
    previewAssetIds: ["histology-section", "visium-spot-array", "segmentation-mask", "neighborhood-graph", "gene-locus"],
    nodeKinds: ["shape", "text", "symbol", "plot", "connector"],
    agentUseHints: ["Use when the user asks for a paper, manuscript, journal-safe figure, or publication panel about spatial transcriptomics.", "Keep image/source provenance and plot source-data notes visible outside the visual panels."],
    qaChecklist: ["No glossy cards, colored UI depth, or decorative shadows.", "Heatmap has axis labels, legend, units, and source-data note.", "Tissue/spot/segmentation/neighborhood panels have clear panel labels."]
  },
  {
  ...
  },
  {
    ...
    description: "Manuscript-safe AI biosecurity evaluation schematic with line-art benchmark, classifier, decision, review/audit, and source-data placeholders.",
    layout: "multi-panel",
    recommendedStyleProfile: "publication-line",
    previewAssetIds: ["dataset", "benchmark", "bio-classifier", "risk-gate", "human-review", "audit-log"],
    nodeKinds: ["shape", "text", "symbol", "plot", "connector"],
    agentUseHints: ["Use when the user asks for a paper, manuscript, journal-safe methods figure, or publication schematic about AI biosecurity evaluation.", "Keep claims descriptive and show source/evaluation data, thresholds, review state, and audit records as separate editable objects."],
    qaChecklist: ["No risk-warning cards, glow, or product-slide depth.", "Risk score plot has axis labels, threshold/legend, and source-data note.", "Human review and audit provenance placeholders are visible."]
  },
  {
...
function createTemplateFigureNodes(pack: WorkflowPack, template: WorkflowTemplate, input: {
  ...
  if (template.id === "spatial-results-panel-journal") return createSpatialResultsJournalTemplateNodes(template, input);
  ...
  if (template.id === "ai-biosecurity-pipeline-journal") return createAiBiosecurityJournalTemplateNodes(template, input);
...
function createSpatialResultsJournalTemplateNodes(template: WorkflowTemplate, input: {
  styleProfile?: AssetStyleProfile;
  x?: number;
  y?: number;
  width?: number;
  ...
  const y = input.y ?? 92;
  const width = input.width ?? 1064;
  const source = curatedProvenance("Synthetic journal-safe spatial transcriptomics figure fixture generated by Scientific Image", "Scientific Image spatial transcriptomics journal template fixture");
  const line: SymbolAppearance = { accent: "#111827", stroke: "#111827", secondary: "#ffffff", fill: "#ffffff", labelColor: "#111827", strokeWidth: 1.35, labelVisible: false };
  const figureText = (value: string, transform: Transform, style: Style = {}, align: "start" | "middle" | "end" = "start"): SceneNode => {
    const node = createTextNode(value, transform, {
      fontFamily: "Arial, Helvetica, sans-serif",
  ...
  const nodes: SceneNode[] = [
    panel(x - 16, y - 48, width + 32, 528),
    figureText("Spatial transcriptomics results schematic", createTransform(x, y - 38, 482, 22), {
      fontSize: 17,
      fontWeight: 850
    }),
    figureText("Journal-safe draft: replace demo tissue, segmentation, and expression table with cited source data.", createTransform(x + 470, y - 35, width - 470, 18), {
      fontSize: 9.6,
      fontWeight: 650,
      color: "#374151"
    }, "end"),
  ...
  const panelC = { x: x + panelA.w + panelB.w + gap * 2, y: topY, w: width - panelA.w - panelB.w - gap * 2, h: panelH };
  for (const [tag, title, p] of [
    ["A", "Tissue and capture array", panelA],
    ["B", "Segmentation and cell boundaries", panelB],
    ["C", "Neighborhood graph", panelC]
  ] as const) {
    nodes.push(panel(p.x, p.y, p.w, p.h));
    nodes.push(figureText(tag, createTransform(p.x + 12, p.y + 10, 20, 18), { fontSize: 15, fontWeight: 850 }));
    nodes.push(figureText(title, createTransform(p.x + 40, p.y + 11, p.w - 52, 18), { fontSize: 11.3, fontWeight: 760 }));
    nodes.push(divider(p.x + 12, p.y + 36, p.w - 24));
  }
  nodes.push(
  ...
  const lowerY = y + 230;
  const resultPanel = { x, y: lowerY, w: 680, h: 218 };
  const reviewPanel = { x: x + resultPanel.w + gap, y: lowerY, w: width - resultPanel.w - gap, h: 218 };
  nodes.push(panel(resultPanel.x, resultPanel.y, resultPanel.w, resultPanel.h));
  nodes.push(figureText("D", createTransform(resultPanel.x + 12, resultPanel.y + 10, 20, 18), { fontSize: 15, fontWeight: 850 }));
  nodes.push(figureText("Marker expression and region summary", createTransform(resultPanel.x + 40, resultPanel.y + 11, 300, 18), { fontSize: 11.5, fontWeight: 760 }));
  nodes.push(divider(resultPanel.x + 12, resultPanel.y + 36, resultPanel.w - 24));
  const heatmapTable = {
    id: createId("table"),
    name: "Journal spatial marker expression table placeholder",
    columns: ["gene", "region", "expression"],
    rows: [
  ...
  nodes.push(createPlotNode({
    ...
    encodings: { x: "region", y: "gene", value: "expression" },
    style: { fill: "#ffffff", stroke: "#111827", color: "#111827", depth: "surface" },
    axes: { x: "Tissue region", y: "Marker gene" },
    legend: "Scaled expression",
    sourceDataNote: "Replace synthetic fixture with source expression table before submission.",
    journalPlot: {
      axisLabels: { x: "Tissue region", y: "Marker gene" },
      units: { value: "scaled expression (a.u.)" },
      legend: "Scaled expression by annotated region",
      sourceData: "source-spatial-expression-table.tsv",
      sourceDataNote: "Synthetic fixture for layout QA; replace with source expression table and analysis version."
    }
  } as never, createTransform(resultPanel.x + 24, resultPanel.y + 52, 430, 146)));
  nodes.push(
    symbol("gene-locus", "Marker gene", resultPanel.x + 490, resultPanel.y + 64, 76, 64, "data-evidence", `${template.id}:panel-d-marker-gene`),
    label("marker gene set", resultPanel.x + 462, resultPanel.y + 132, 132),
    figureText("Report normalization, region annotation, and multiple-testing procedure with source table.", createTransform(resultPanel.x + 466, resultPanel.y + 154, 188, 34), {
      fontSize: 8.6,
      color: "#374151"
    })
  );
  ...
  nodes.push(divider(reviewPanel.x + 12, reviewPanel.y + 36, reviewPanel.w - 24));
  const checks = [
    ["Tissue image", "Attach raw image, stain/platform, crop, and processing notes."],
    ["Segmentation", "Record model/tool, QC filters, and manual correction status."],
    ["Expression", "Attach source-spatial-expression-table.tsv and region annotation version."],
    ["Legend", "Define scale, region classes, and color mapping in caption."]
  ];
  checks.forEach(([title, detail], index) => {
    const rowY = reviewPanel.y + 54 + index * 38;
    nodes.push(figureText(`${index + 1}`, createTransform(reviewPanel.x + 20, rowY, 18, 16), { fontSize: 9.5, fontWeight: 850 }));
    nodes.push(figureText(title, createTransform(reviewPanel.x + 48, rowY - 1, 86, 16), { fontSize: 9.5, fontWeight: 800 }));
    nodes.push(figureText(detail, createTransform(reviewPanel.x + 140, rowY - 1, reviewPanel.w - 162, 28), { fontSize: 8.5, color: "#374151" }));
  });
  nodes.push(figureText("Caption placeholder: Spatial transcriptomics map links tissue regions, segmentation-derived neighborhoods, and marker expression; replace demo values with cited source data.", createTransform(x, y + 458, width, 34), {
    fontSize: 9.2,
    fontWeight: 650,
    color: "#374151"
  }));
...
function createAiBiosecurityJournalTemplateNodes(template: WorkflowTemplate, input: {
  styleProfile?: AssetStyleProfile;
  x?: number;
  y?: number;
  width?: number;
  ...
  const y = input.y ?? 92;
  const width = input.width ?? 1064;
  const source = curatedProvenance("Synthetic journal-safe AI biosecurity evaluation metrics generated by Scientific Image", "Scientific Image AI biosecurity journal template fixture");
  const line: SymbolAppearance = { accent: "#111827", stroke: "#111827", secondary: "#ffffff", fill: "#ffffff", labelColor: "#111827", strokeWidth: 1.35, labelVisible: false };
  const figureText = (value: string, transform: Transform, style: Style = {}, align: "start" | "middle" | "end" = "start"): SceneNode => {
    const node = createTextNode(value, transform, {
      fontFamily: "Arial, Helvetica, sans-serif",
  ...
  const nodes: SceneNode[] = [
    panel(x - 16, y - 48, width + 32, 528),
    figureText("AI biosecurity evaluation schematic", createTransform(x, y - 38, 448, 22), {
      fontSize: 17,
      fontWeight: 850
    }),
    figureText("Journal-safe draft: replace demo benchmark, thresholds, and reviewer policy notes with cited source material.", createTransform(x + 438, y - 35, width - 438, 18), {
      fontSize: 9.6,
      fontWeight: 650,
      color: "#374151"
    }, "end"),
  ...
  const panelH = 178;
  const gap = 18;
  const panelA = { x, y: topY, w: 306, h: panelH };
  const panelB = { x: x + panelA.w + gap, y: topY, w: 338, h: panelH };
  const panelC = { x: x + panelA.w + panelB.w + gap * 2, y: topY, w: width - panelA.w - panelB.w - gap * 2, h: panelH };
  for (const [tag, title, p] of [
    ["A", "Source and benchmark set", panelA],
    ["B", "Classifier and calibration", panelB],
    ["C", "Permission decision path", panelC]
  ] as const) {
    nodes.push(panel(p.x, p.y, p.w, p.h));
    nodes.push(figureText(tag, createTransform(p.x + 12, p.y + 10, 20, 18), { fontSize: 15, fontWeight: 850 }));
    nodes.push(figureText(title, createTransform(p.x + 40, p.y + 11, p.w - 52, 18), { fontSize: 11.3, fontWeight: 760 }));
  ...
  nodes.push(
    symbol("dataset", "Source prompt set", panelA.x + 28, panelA.y + 54, 82, 68, "data-evidence", `${template.id}:panel-a-source-set`),
    label("source set", panelA.x + 18, panelA.y + 128, 104),
    ...
    symbol("bio-classifier", "Bio classifier", panelB.x + 34, panelB.y + 52, 94, 72, "model-checkpoint", `${template.id}:panel-b-classifier`),
    label("classifier", panelB.x + 24, panelB.y + 128, 108),
    symbol("calibration", "Calibration curve", panelB.x + 150, panelB.y + 52, 84, 72, "evaluation-evidence", `${template.id}:panel-b-calibration`),
    label("calibration", panelB.x + 138, panelB.y + 128, 108),
    symbol("metric-card", "Metric card", panelB.x + 254, panelB.y + 54, 64, 66, "evaluation-evidence", `${template.id}:panel-b-metric-card`),
    ...
    symbol("risk-gate", "Risk gate", panelC.x + 28, panelC.y + 52, 78, 70, "risk-decision", `${template.id}:panel-c-risk-gate`),
    label("risk gate", panelC.x + 16, panelC.y + 128, 102),
    symbol("permission-tier", "Permission tier", panelC.x + 132, panelC.y + 52, 88, 70, "risk-decision", `${template.id}:panel-c-permission-tier`),
    label("permission tier", panelC.x + 116, panelC.y + 128, 118),
    symbol("human-review", "Human review", panelC.x + 246, panelC.y + 52, 84, 70, "risk-decision", `${template.id}:panel-c-human-review`),
  ...
  const lowerY = y + 230;
  const resultPanel = { x, y: lowerY, w: 682, h: 218 };
  const reviewPanel = { x: x + resultPanel.w + gap, y: lowerY, w: width - resultPanel.w - gap, h: 218 };
  nodes.push(panel(resultPanel.x, resultPanel.y, resultPanel.w, resultPanel.h));
  nodes.push(figureText("D", createTransform(resultPanel.x + 12, resultPanel.y + 10, 20, 18), { fontSize: 15, fontWeight: 850 }));
  nodes.push(figureText("Evaluation score and threshold summary", createTransform(resultPanel.x + 40, resultPanel.y + 11, 328, 18), { fontSize: 11.5, fontWeight: 760 }));
  nodes.push(divider(resultPanel.x + 12, resultPanel.y + 36, resultPanel.w - 24));
  const metricTable = {
    id: createId("table"),
    name: "Journal AI biosecurity evaluation metrics placeholder",
    columns: ["class", "score", "threshold", "split"],
    rows: [
      { class: "benign protocol", score: 0.12, threshold: 0.5, split: "test" },
      { class: "dual-use ambiguity", score: 0.61, threshold: 0.5, split: "test" },
  ...
  nodes.push(createPlotNode({
    ...
    encodings: { x: "class", y: "score", color: "split" },
    style: { fill: "#ffffff", stroke: "#111827", color: "#111827", depth: "surface" },
    axes: { x: "Evaluation class", y: "Classifier score" },
    legend: "Evaluation split and decision threshold",
    sourceDataNote: "Replace synthetic fixture with source evaluation table before submission.",
    journalPlot: {
      axisLabels: { x: "Evaluation class", y: "Classifier score" },
      units: { y: "score, 0-1" },
      legend: "Test split; horizontal threshold = 0.5",
      sourceData: "source-ai-biosecurity-eval-metrics.tsv",
      sourceDataNote: "Synthetic fixture for layout QA; replace with source benchmark and threshold table."
    }
  } as never, createTransform(resultPanel.x + 24, resultPanel.y + 52, 432, 146)));
  nodes.push(
    symbol("durc-flag", "DURC flag", resultPanel.x + 492, resultPanel.y + 62, 70, 66, "risk-decision", `${template.id}:panel-d-durc-flag`),
    label("flag class", resultPanel.x + 466, resultPanel.y + 132, 120),
    symbol("audit-log", "Audit log", resultPanel.x + 584, resultPanel.y + 62, 70, 66, "governance-record", `${template.id}:panel-d-audit-log`),
    label("audit record", resultPanel.x + 560, resultPanel.y + 132, 116),
    figureText("Report threshold source, uncertainty, and reviewer disposition with the evaluation table.", createTransform(resultPanel.x + 466, resultPanel.y + 154, 190, 34), {
      fontSize: 8.5,
      color: "#374151"
    })
  );
  ...
  nodes.push(panel(reviewPanel.x, reviewPanel.y, reviewPanel.w, reviewPanel.h));
  nodes.push(figureText("E", createTransform(reviewPanel.x + 12, reviewPanel.y + 10, 20, 18), { fontSize: 15, fontWeight: 850 }));
  nodes.push(figureText("Review and reproducibility checklist", createTransform(reviewPanel.x + 40, reviewPanel.y + 11, reviewPanel.w - 60, 18), { fontSize: 11.5, fontWeight: 760 }));
  nodes.push(divider(reviewPanel.x + 12, reviewPanel.y + 36, reviewPanel.w - 24));
  const checks = [
  ...
  checks.forEach(([title, detail], index) => {
    const rowY = reviewPanel.y + 54 + index * 38;
    nodes.push(figureText(`${index + 1}`, createTransform(reviewPanel.x + 20, rowY, 18, 16), { fontSize: 9.5, fontWeight: 850 }));
    nodes.push(figureText(title, createTransform(reviewPanel.x + 48, rowY - 1, 82, 16), { fontSize: 9.5, fontWeight: 800 }));
    nodes.push(figureText(detail, createTransform(reviewPanel.x + 136, rowY - 1, reviewPanel.w - 158, 28), { fontSize: 8.5, color: "#374151" }));
  });
  nodes.push(figureText("Caption placeholder: Evaluation pipeline links source set, benchmark labels, classifier score, permission tier, human review, and audit record; replace demo values with cited source data.", createTransform(x, y + 458, width, 34), {
    fontSize: 9.2,
    fontWeight: 650,
    color: "#374151"
  }));
...
function recommendedTemplateForIntent(workflowPack: string | undefined, intent: string): string | undefined {
  ...
  if (workflowPack === "spatial-transcriptomics" && prefersJournalFigureForIntent(intent)) return "spatial-results-panel-journal";
  if (workflowPack === "ai-biosecurity-eval" && prefersJournalFigureForIntent(intent)) return "ai-biosecurity-pipeline-journal";
  return undefined;
}
