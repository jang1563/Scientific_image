...
| Can a reviewer open public examples quickly? | README direct demo links and `apps/web/src/app.js` public demo launcher | `node scripts/serve-static.ts apps/web 4173`, then open `http://127.0.0.1:4173/?demo=perturb-seq-workflow` or `http://127.0.0.1:4173/?demo=perturb-seq-workflow-journal` |
| What does each public visual example prove? | `docs/PUBLIC_EXAMPLE_GALLERY.md`, `docs/examples/manifest.json`, `docs/examples/*.svg` | `node scripts/generate-public-examples.ts` and `git diff --exit-code docs/examples` |
