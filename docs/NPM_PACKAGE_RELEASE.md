...
Users can verify MCP startup before editing Claude/Codex config:
...
- `dist/` runtime JS generated by `node scripts/build-npm-package.ts`
...
Ignored local artifacts such as `output/`, `.scientific-image/`, `.playwright-cli/`, `node_modules/`, and environment files must stay out of the tarball. The generated `dist/` directory is allowed in the npm tarball but remains untracked in git.
