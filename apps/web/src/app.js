...
function renderVolcanoPlot(points, x, y, width, height, theme) {
  ...
  const labels = hitPoints
    .map((point, index) => {
      ...
      const journalAttrs = isPublication ? ` data-journal-label-side="${point.x > 0 ? "up" : "down"}"` : "";
      return `${leader}<text class="plot-volcano-label${isPublication ? " plot-journal-volcano-label" : ""}"${journalAttrs} x="${labelX}" y="${labelY}" text-anchor="${placeLeft ? "end" : "start"}" font-size="8.4" font-weight="${isPublication ? "690" : "760"}" fill="${theme.label}">${escapeXml(shortPlotLabel(point.label, 8))}</text>`;
    })
    .join("");
...
function pickJournalVolcanoLabels(points, limit) {
  const selected = [];
  for (const side of ["up", "down"]) {
    const point = points.find((candidate) => side === "up" ? candidate.x > 0 : candidate.x < 0);
  ...
  return selected.sort((a, b) => b.y - a.y);
}
