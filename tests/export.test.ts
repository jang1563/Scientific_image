...
test("spatial transcriptomics journal template exports manuscript-safe line figure", () => {
  ...
  assert.match(svg, /plot-axis-label-x/);
  assert.match(svg, /plot-axis-label-y/);
  assert.match(svg, /Tissue region/);
  assert.match(svg, /Marker gene/);
  assert.match(svg, /plot-journal-legend/);
...
test("AI biosecurity journal template exports manuscript-safe line figure", () => {
  ...
  assert.match(svg, /plot-axis-label-x/);
  assert.match(svg, /plot-axis-label-y/);
  assert.match(svg, /M48,88 H414 M48,88 V28/);
  assert.match(svg, /Evaluation class/);
  ...
  assert.match(svg, /plot-journal-legend/);
  assert.match(svg, /Test split; horizontal threshold = 0\.5/);
  assert.match(svg, /plot-source-data-note/);
  assert.match(svg, /plot-compact-footer/);
